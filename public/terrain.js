...
var mySVG = bodySelection.append("svg")
       .attr("width", 2000)
       .attr("height", 2000);

//this is the amount of map that gets generated? not sure
var defaultExtent = {
    width: 2.5,
    height: 2.5
};
// this is the scale of the resulting map
//how many units are crammed into the widht & height
//so the bigger the # the smaller things look!
var mapScale = 1000;

var defaultParams = {
    extent: defaultExtent,
    generator: generateCoast,
    npts: 10000,//npts: 16384,
    ncities: 15,
    nterrs: 5,
    fontsizes: {
        region: 40,
...
function getNorthSlope(h) {
    var dh = downhill(h);
    var northSlope = zero(h.mesh);
    for (var i = 0; i < h.length; i++) {


        //var s = trislope(h, i);////////////////////remove?
        //northSlope[i] = s[0];//////////////////////remove?
        //continue;////////////////
        if (h[i]>0) {
            //height > 0 - above sea level
                if (dh[i] && h[dh[i]]) {
                //console.log('h[i]: '+ h[i] +'and h[dh[i]]: '+h[dh[i]]);
                //console.log('So h diff is: '+ (h[i] - h[dh[i]]));
                //console.log('And yDistance is : '+ yDistance(h.mesh, i, dh[i]));

                if (dh[i] < 0) {//dh[i] exists 
                    northSlope[i] = 0;
                } else {
                    northSlope[i] = (h[i] - h[dh[i]]) / yDistance(h.mesh, i, dh[i]);
                }//end else dh > 0
            } else {
                //console.log('dh[i] or h[dh[i]] doesnt exist so h diff doesnt matter ');
                northSlope[i] = 0;
            }//end else dh[i] exists at all
        } else {
                northSlope[i] = 99;
        }//end h[i]>0 negative heights


    //console.log('So northSlope[i] is : '+ northSlope[i]);
...
function mountUp(h, amount) {
    //var newh = zero(h.mesh);
    //var newh = h;

    for (var i = 0; i < 2000; i++) {
        var someSpot = Math.floor(runif(0,h.length));
        var oldH = h[someSpot];
        var maxHi = 1-oldH;
        var addHi = (maxHi/10)*amount;
        h[someSpot]=oldH+addHi;
        //console.log("point#: "+someSpot+" oldH was: "+oldH+" but now adding: "+addHi+" for new h of: "+h[someSpot]);
    }//end for
...
function fracSeg(path, beginPt, endPt, fracLevel) {
        console.log ("in fracSeg with fracLevel = "+fracLevel);
        if (fracLevel>0) {
            //do frac
            var split1 = runif(0,3);//some value between 1 & 3 (which will be used for a point between 0 and 1/3rd of the way along the original segment)
            var split2 = runif(6,10);//some value between 6 & 10 (which will be used for a point between 2/3rd and the end of the original segment)
            var midPt = [(beginPt[0]+endPt[0])/2,(beginPt[1]+endPt[1])/2];//the middle of the current segment
            var x10th = (endPt[0]-beginPt[0])/10; //1/10th the x distance of the current segment
            var y10th = (endPt[1]-beginPt[1])/10; //1/10th the y distance of the currrent seg
            var pt2 = [(beginPt[0]+split1*x10th),(beginPt[1]+split1*y10th)];//a new point split1 10ths along the current seg
            var pt4 = [(beginPt[0]+split2*x10th),(beginPt[1]+split2*y10th)];//a new point split2 10ths along the current seg
            var pt3 = [runif(pt2[0], pt4[0]),runif(pt2[1], pt4[1])]; //random pt in the "box" between pt2 & pt3 
        console.log ("split1: "+split1);
        console.log ("split2: "+split2);
        console.log ("midPt: "+midPt);
        console.log ("x10th: "+x10th);
        console.log ("y10th: "+y10th);
        console.log ("beginPt: "+beginPt);
        console.log ("pt2: "+pt2);
        console.log ("pt3: "+pt3);
        console.log ("pt4: "+pt4);
        console.log ("endPt: "+endPt);

            fracSeg(path, beginPt, pt2, fracLevel-1);//draw first subsegment 
            fracSeg(path, pt2, pt3, fracLevel-1);//draw second subsegment
            fracSeg(path, pt3, pt4, fracLevel-1);//draw third subsegemnt
            fracSeg(path, pt4, endPt, fracLevel-1);//draw last subsegment
        } else {
            //if frac level is 0 (or we've recursiveley run it down to 0) then just draw from begin to end
            console.log ("about to add ["+endPt[0]+", "+endPt[1]+"]");
            path.push[endPt];
        }
}//end fracSeg

function fractalize(path, fracLevel) {
    fracLevel = fracLevel || 2;
    var newPath = [];
    newPath.push(path[0]);
    for (var i = 1; i < path.length; i++) {
        fracSeg(newPath, path[i-1], path[i], fracLevel);
    }
    return newPath;
}
...
function visualizeVoronoi(svg, field, lo, hi) {
    ...
    svg.selectAll('path.field')
        ...
        .style('fill', function (d, i) {
            ...
            return d3.interpolateCubehelixDefault(mappedvals[i]);
        });
}//end visualizeVoronoi

function shadeVoronoi(svg, field, lo, hi) {
...
function visualizeSlopes(svg, render) {
    ...
    ///Make strokes for every mesh area/////////////////
    for (var i = 0; i < h.length; i++) {
...
function northLightShading1(svg, render) {

    //if (hi == undefined) hi = d3.max(field) + 1e-9;
   // if (lo == undefined) lo = d3.min(field) - 1e-9;
   var hi = 0;
   var lo = -4;
    var NSlope = getNorthSlope(render.h);
    var mappedvals = NSlope.map(function (x) {return x > hi ? 1 : x < lo ? 0 : (x - lo) / (hi - lo)});
    var tris = svg.selectAll('path.field').data(render.h.mesh.tris)
    var color = d3.scaleLinear().domain([0,1])
      .interpolate(d3.interpolateHcl)
      .range([d3.rgb("#999999"), d3.rgb('#FFFFFF')]);
...
}//end northLightShading1

function northLightShading2(svg, render) {
    var h = render.h;
    //var dots = [];
    var r = 0.25 / Math.sqrt(h.length);
    var NSlope = getNorthSlope(render.h);
    var spread = 0.02;//////should be a function of dist to neighbors
    ...
    for (var i = 0; i < h.length; i++) {
        ...
        var x = h.mesh.vxs[i][0];
        var y = h.mesh.vxs[i][1];
        //make some dots around there according to slope
        for (var dotCnt = 0; dotCnt < NSlope[i]; dotCnt++) {
            var jiggleX = x+runif(-1*spread, spread);
            var jiggleY = y+runif(-1*spread, spread);
            dots.push([jiggleX,jiggleY]);
        }//end for dotCnt making dots
    }// end for i < h.length

    var theDots = svg.selectAll('dots.slope').data(dots)
    theDots.enter()
            .append('dots')
            .classed('slope', true);
    theDots.exit()
...
*/
    svg.selectAll('dots')
        .attr('cx', function (d) {return d[0]*mapScale * render.params.extent.width})
        .attr('cy', function (d) {return d[1]*mapScale * render.params.extent.height})
        .attr('r', 3)
        .style('fill', '#333333')
        .style('stroke', 'none')


        //.attr('r', spread/10*mapScale)

}//end northLightShading2


function northLightShading3(svg, render) {
    ...
    for (var i = 0; i < h.length; i++) {
        if (h[i] <= 0 || isnearedge(h.mesh, i)) continue;
        var spread = distance(h.mesh, i, h.mesh.adj[i][0])*1.2;////////maybe?
        var aSmallLngth = spread/30;///maybe
        //where are we?
        var x = h.mesh.vxs[i][0];
        var y = h.mesh.vxs[i][1];
        //make some dots around there according to slope
        for (var dotCnt = 0; dotCnt < NSlope[i]; dotCnt++) {
            var jiggleX = x+runif(-1*spread, spread);
            var jiggleY = y+runif(-1*spread, spread);
            var jiggleX2 = jiggleX+runif(-1*aSmallLngth, aSmallLngth);
            var jiggleY2 = jiggleY+runif(-1*aSmallLngth, aSmallLngth);
            strokes.push([[jiggleX,jiggleY], [jiggleX2,jiggleY2]]);
    ...
    svg.selectAll('line.slope')
        ...
        .attr('x2', function (d) {return 1000*d[1][0]})
        .attr('y2', function (d) {return 1000*d[1][1]})
}//end northLightShading3


function northLightShading4(svg, render) {
    var h = render.h;
    var strokes = [];
    var r = 0.25 / Math.sqrt(h.length);
    var dh = downhill(h);
    ...
    for (var i = 0; i < h.length; i++) {
        ...
        var up = h.mesh.vxs[i];
        var down = h.mesh.vxs[dh[i]];
        if (down) {down = [(down[0]+up[0]*3)/4, (down[1]+up[1]*3)/4 ]};/////shorter???
        ...
        //if (Math.abs(l*s) > 2 * r) {
            //console.log("s is:"+s2);
        if (s2<0 && down) {
            var n = Math.floor(Math.abs(s2));
            //l /= n;
            //if (n > 4) n = 4;
            for (var j = 0; j < n*2; j++) {
                var u = runif(-3,3) * r;
                var v = runif(-3,3) * r;
                var l = r * runif(1, 3) * (1 - 0.2 * Math.pow(Math.atan(s), 2)) * Math.exp(s2/100);
                //strokes.push([[x+u-l*s, y+v+l*s2], [x+u+l*s, y+v-l*s2]]);
                strokes.push([[up[0]+u-l*s, up[1]+v+l*s2], [down[0]+u+l*s, down[1]+v-l*s2]]);
            }
        } else {
            //strokes.push([[x-l, y+l*s], [x+l, y-l*s]]);
        }
    ...
    svg.selectAll('line.slope')
        ...
        .attr('x2', function (d) {return 1000*d[1][0]})
        .attr('y2', function (d) {return 1000*d[1][1]})
}//end northLightShading4

function pushHatch(hatchArr, x, y, size) {

    var rx = size;
    var ry = .75*size;
    hatchArr.push([[x-.25*rx, y-.5*ry], [x+.5*rx, y+.25*ry]]);
    hatchArr.push([[x-.5*rx, y-.25*ry], [x+.25*rx, y+.5*ry]]);
    hatchArr.push([[x-.5*rx, y+.25*ry], [x+.25*rx, y-.5*ry]]);
    hatchArr.push([[x-.25*rx, y+.5*ry], [x+.5*rx, y-.25*ry]]);


}//end pushHatch2Strokes


function northLightShading5(svg, render) {
    var h = render.h;
    var strokes = [];
    var r = 1.5 / Math.sqrt(h.length);
    ...
    for (var i = 0; i < h.length; i++) {
        ...
        //if (Math.abs(l*s) > 2 * r) {
            //console.log("s is:"+s2);
        if (s2<0) {
            var hatchCnt = Math.floor(Math.abs(s2));
            //l /= n;
            //if (n > 4) n = 4;
            for (var j = 0; j < hatchCnt; j++) {
                var jiggleX = runif(-.5,.5) * r;
                var jiggleY = runif(-.5,.5) * r;
                pushHatch(strokes, x+jiggleX, y+jiggleY, r);
            }
        } else {
            //strokes.push([[x-l, y+l*s], [x+l, y-l*s]]);
        }
...
function northLightShading6(svg, render) {
/* --------------------
      Textures
    -------------------- */
    ...
    svg
      ...
      .append('pattern')
        ...
        .attr('height', 10)
      .append('path')
        .attr("d", "M3,5 L0,6 M4,7 L4,8 M8,2 L9,3")
        .attr('fill', 'none')
        .attr('stroke-width', '1')
        .attr('stroke', '#ccc');
    ...
    svg
      ...
      .append('pattern')
        ...
        .attr('height', 10)
      .append('path')
        .attr("d", "M3,2 L3,5 L0,6 M4,3 L4,7 L4,8 M8,2 L9,3 L8,6")
        .attr('fill', 'none')
        .attr('stroke-width', '1')
        .attr('stroke', '#ccc');
    ...
    svg
      ...
      .append('pattern')
        ...
        .attr('height', 10)
      .append('path')
        .attr("d", "M3,0 L2,3 L3,6 M3,3 L3,7 L4,10 M5,0 L6,5 M7,4 L6,7 L 8,10 M10,3 L8,5")
        .attr('fill', 'none')
        .attr('stroke-width', '1')
        .attr('stroke', '#ccc');
    ...
    svg
      ...
      .append('pattern')
        ...
        .attr('height', 10)
      .append('path')
        .attr("d", "M3,0 L2,3 L3,6 L0,10 M4,0 L3,3 L3,7 L4,10 M5,0 L6,5 L5,10 M7,0 L6,1 L7,4 L6,7 L 8,10 M8,0 L10,3 L8,5 L9,7")
        .attr('fill', 'none')
        .attr('stroke-width', '1')
        .attr('stroke', '#ccc');
   ...
   var lo = -4;
    ...
    svg.selectAll('path.field')
        ...
        .style('fill', function (d, i) {
            ...
            //return d3.interpolateCubehelixDefault(mappedvals[i]);
            //return color(mappedvals[i]);
            var _ran = Math.floor( mappedvals[i] * 4 );
            return 'url(#texture' + _ran + ')';
        });
}//end northLightShading6
...
function generateCoast(params) {
    ...
    var h = add(
            slope(mesh, randomVector(4)),//4
            cone(mesh, runif(-1, -1)),
            mountains(mesh, 200, .02)//50
            );
    for (var i = 0; i < 10; i++) {
        h = relax(h);
    }
...
function drawMap(svg, render) {
    ...
    render.rivers = getRivers(render.h, 0.001);//.01 the higher the less rivers
    render.coasts = contour(render.h, 0);
    //render.coasts = fractalize(render.coasts, 2);////////////experiment to rough up coast path
    //render.terr = getTerritories(render);
    //render.borders = getBorders(render);


    //visualizeVoronoi(mySVG, render.h);////shades triangles
    svg.selectAll('line.slope')
    .attr("stroke", "#AAAAAA")
    .attr("stroke-width", 0.5)
    .attr("fill", "none");
    //visualizeSlopes(svg, render);
    northLightShading6(svg, render);
    //visualizeDownhill(mySVG, render.h);
...
//    };
    ...
    svg.attr('viewBox', -1*mapScale * params.extent.width/2 + ' ' + 
                        -1*mapScale * params.extent.height/2 + ' ' + 
                        mapScale * params.extent.width + ' ' + 
                        mapScale * params.extent.height);
...
function beginMap() {
    ...
    var inputSvgW = document.getElementById("svgW");
    var inputSvgH = document.getElementById("svgH");

    var inputExtentW = document.getElementById("extentW");
    var inputExtentH = document.getElementById("extentH");
 
    var inputScale = document.getElementById("scale");

    var pointCount = document.getElementById("points");

    mySVG.attr("width", inputSvgW.value);
    mySVG.attr("height", inputSvgH.value);

    //this is the amount of map that gets generated? not sure
    defaultExtent.width = inputExtentW.value;
    defaultExtent.height = inputExtentH.value;

    // this is the scale of the resulting map
    //how many units are crammed into the widht & height
    //so the bigger the # the smaller things look!
    mapScale = inputScale.value;
    defaultParams.npts = pointCount.value;

    doMap(mySVG, defaultParams);

    if (displayVor.checked == true)
    {
        visualizeVoronoi(mySVG, render.h);
    } else {
        drawMap(mySVG, render);
    };//end else

   // getNorthSlope(render.h);//////////////////////////////////////////////////////

}//end beginMap

var displayVor = document.getElementById("voronoi");

function erodeButt() {
    console.log("clicked erode");
d3.selectAll("svg > *").remove()

render.h = doErosion(render.h, runif(0, 0.1), 5);
//render.h = cleanCoast(render.h, 3);
...
function fillSinksButt() {
console.log("clicked fillSinks");
d3.selectAll("svg > *").remove()

render.h = fillSinks(render.h);
render.h = cleanCoast(render.h, 3);
...
function resetSea() {
console.log("clicked resetSea");
d3.selectAll("svg > *").remove()

render.h = setSeaLevel(render.h, runif(0.2, 0.6));
render.h = cleanCoast(render.h, 3);
...
function cities() {
console.log("clicked cities");
d3.selectAll("svg > *").remove()

render.cities = [];
placeCities(render);
visualizeCities(mySVG, render);

    if (displayVor.checked == true)
    {
        //visualizeVoronoi(mySVG, render.h);
        northLightShading1(mySVG, render);
    } else {
        drawMap(mySVG, render);
    };//end else
    }//end temp

function freshMountains() {
console.log("clicked freshMountains");
d3.selectAll("svg > *").remove()

//render.h = mountUp(render.h, 600);
mountUp(render.h, 8);
//render.h = add(mountains(render.h.mesh, 100, .03));//.05 is the default
//render.h = cleanCoast(render.h, 3);
...
function peakyButt() {
console.log("clicked peaky");
d3.selectAll("svg > *").remove()

render.h = peaky(render.h);
render.h = cleanCoast(render.h, 3);
...
var url = "http://localhost:3000/crap";
...
var data = JSON.stringify(render);
...
function loadMap(selectedMap) {
   var request = new XMLHttpRequest();
    ...
    selectedMap = 11;
    //request.open("GET", "http://localhost:3000/shit?map="+selectedMap, true);
    ...
    request.onreadystatechange = function() {
         if (request.readyState === 4 && request.status === 200) {
            try {
                ...
                var loadedRender = JSON.parse(response[0].MapData);
                 console.log(loadedRender);
                 render = loadedRender;
                 d3.selectAll("svg > *").remove()
